...
func main() {
	...
	rows, err := conn.Query(context.Background(), "SELECT tablename, policyname, cmd, qual, with_check FROM pg_policies WHERE schemaname = 'public' AND tablename IN ('exams', 'questions')")
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()

	fmt.Println("RLS Policies for exams and questions:")
	for rows.Next() {
