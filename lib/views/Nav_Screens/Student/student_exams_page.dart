  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        return FutureBuilder<List<Map<String, dynamic>?>>(
          future: Future.wait(rawExams.map((e) => _supabaseService.checkSubmission(e['id']))),
          builder: (context, submissionSnapshot) {
            final submissions = submissionSnapshot.data ?? [];
            
            final List<Map<String, dynamic>> exams = rawExams.asMap().entries.map((entry) {
              final i = entry.key;
              final e = entry.value;
              ...
              final submission = i < submissions.length ? submissions[i] : null;
              
              // Calculate real-time status based on date/time
              String calculatedStatus = e['status'] ?? 'Upcoming';
              try {
                if (e['exam_date'] != null) {
                  final examDateTime = DateTime.parse(e['exam_date']);
                  if (DateTime.now().isAfter(examDateTime)) {
                    calculatedStatus = 'Active';
                  }
                }
              } catch (e) {
                // Fallback to DB status if parsing fails
              }
              ...
              return {
                ...
                'questions': (e['questions'] as List?)?.length ?? 0,
                'status': submission != null ? 'Completed' : calculatedStatus,
                      ...
                      children: [
                        ...
                        Column(
                          children: filtered.asMap().entries.map((entry) {
                            ...
                            final subStatus = exam['submission_status'] as String?;
                            final sColor = _statusColor(status);
                            final isActive = status == 'Active';
                            final isCompleted = status == 'Completed';
                            final isTerminated = subStatus == 'terminated';
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        Container(
                                          ...
                                          width: 52,
                                          decoration: BoxDecoration(
                                            color: isTerminated ? Colors.red.shade100 : exam['iconbg'] as Color,
                                            borderRadius: BorderRadius.circular(12),
                                          ),
                                          child: Icon(
                                            isTerminated ? FontAwesomeIcons.circleExclamation : exam['icon'] as IconData,
                                            color: isTerminated ? Colors.red : exam['iconcolor'] as Color,
                                            size: 22,
                                          ),
                                        ),
                                        ...
                                        Column(
                                          ...
                                          children: [
                                            Container(
                                              ...
                                              decoration: BoxDecoration(
                                                color: (isTerminated ? Colors.red : sColor).withOpacity(0.1),
                                                borderRadius: BorderRadius.circular(
                                                  20,
                                                ),
                                              ),
                                              child: Text(
                                                isTerminated ? 'Terminated' : status,
                                                style: TextStyle(
                                                  fontSize: 12,
                                                  fontWeight: FontWeight.bold,
                                                  color: isTerminated ? Colors.red : sColor,
                                                ),
                                              ),
                                            ),
                                            ...
                                            else if (isCompleted || isTerminated)
                                              Text(
                                                isTerminated ? 'No Retakes' : 'Submitted',
                                                style: TextStyle(
                                                  fontSize: 14,
                                                  fontWeight: FontWeight.bold,
                                                  color: isTerminated ? Colors.red : AppColor.primaryBlue,
                                                ),
                                              )
                                            else if (isActive)
                                              ElevatedButton(
